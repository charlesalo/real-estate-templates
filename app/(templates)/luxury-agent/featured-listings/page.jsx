        ...
        <div className="group border border-white/[0.07] overflow-hidden hover:border-white/15 transition-colors grid lg:grid-cols-2 gap-0">
          ...
          <Link href={`/luxury-agent/featured-listings/${hero.id}`} className="relative aspect-[4/3] lg:aspect-auto lg:min-h-[520px] overflow-hidden block">
            ...
            <div className="absolute top-6 left-6">
              <span className={`inline-block px-3 py-1.5 text-[12px] tracking-[0.25em] uppercase font-semibold ${BADGE_STYLES[hero.badge] ?? 'bg-white/15 text-white backdrop-blur-sm'}`}>
                {hero.badge}
              </span>
            </div>
          </Link>
          ...
          <div className="bg-[#0D0D0D] p-8 lg:p-12 flex flex-col justify-center">
            <Link href={`/luxury-agent/featured-listings/${hero.id}`} className="block">
              <p className="text-[12px] tracking-[0.4em] uppercase text-[#C9A96E] mb-4 font-sans">
            ...
            <ShowingTrigger listing={hero} className="self-start px-8 py-4 text-[12px] tracking-[0.2em] uppercase font-medium bg-[#C9A96E] text-[#0A0A0A] hover:opacity-90 transition-opacity">
      ...
      <Link href={`/luxury-agent/featured-listings/${listing.id}`} className="block">
        ...
        <div className="relative aspect-[16/9] overflow-hidden">
          ...
          <div className="absolute top-4 left-4">
            <span className={`inline-block px-2.5 py-1 text-[12px] tracking-[0.25em] uppercase font-semibold ${BADGE_STYLES[listing.badge] ?? 'bg-white/15 text-white backdrop-blur-sm'}`}>
              {listing.badge}
            </span>
          </div>
        ...
        <div className="bg-[#0D0D0D] p-6 pb-4">
          <p className="text-[12px] tracking-[0.3em] uppercase text-[#C9A96E] mb-1 font-sans">
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            {listing.features.slice(0, 3).map(f => (
              <span key={f} className="px-2.5 py-1 text-[12px] border border-white/10 text-white/35 font-sans">
                {f}
              </span>
            ))}
            {listing.features.length > 3 && (
              <span className="px-2.5 py-1 text-[12px] text-white/20 font-sans">
