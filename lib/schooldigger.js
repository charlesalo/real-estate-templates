...
export async function getSchools(zipCode, state = 'TX') {
  ...
  try {
    ...
    return list.map(s => {
      const address = [s.address?.street, s.address?.city, s.address?.state, s.address?.zip]
        .filter(Boolean).join(', ')
      return {
      id:         s.schoolid,
      name:       s.schoolName,
      address,
      // SchoolDigger has no Google place_id, so anchor a Maps search on the
      // school name + written address.
      mapsUrl:    `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent([s.schoolName, address].filter(Boolean).join(' '))}`,
